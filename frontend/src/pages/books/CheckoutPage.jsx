...
const CheckoutPage = () => {
    ...
    // Helper for showing items with quantity
    const renderCartSummary = () => {
        if (!cartItems.length) return "No items";
        return cartItems.map((item, idx) =>
            <span key={item._id || idx}>
                {item.title} <span className="text-xs text-gray-500">({item.quantity || 1})</span>
                    ...
                    <div>
                        <div>
                            ...
                            <p className="text-gray-500 mb-6">
                                Total Quantity: {cartItems.reduce((acc, item) => acc + (item.quantity || 1), 0)}
                            </p>
                        </div>
...
// const CheckoutPage = () => {
//     const cartItems = useSelector(state => state.cart.cartItems);
//     // Fix: Calculate total price with quantity
//     const totalPrice = cartItems.reduce((acc, item) => acc + (item.newPrice * (item.quantity || 1)), 0).toFixed(2);
//     const {  currentUser} = useAuth()
//     const {
//         register,
